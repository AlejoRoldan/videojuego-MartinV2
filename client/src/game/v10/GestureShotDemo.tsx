...
function getResultCopy(result: ShotPhysicsResult) {
  if (result.goalMouthResult === "goal") return { title: "¡GOL!", detail: "Tu gesto y la trayectoria coincidieron.", accent: "#72f2a1" };
  if (result.goalMouthResult === "high") return { title: "¡Muy alto!", detail: "Prueba un deslizamiento menos vertical.", accent: "#ffd166" };
  if (result.goalMouthResult === "wide") return { title: "¡Afuera!", detail: "Reduce la dirección lateral o el efecto.", accent: "#ff9f68" };
...
export default function GestureShotDemo() {
  const initialResult = useMemo(() => simulateShot(DEFAULT_CONFIG), []);
  ...
  const animate = (now: number) => {
    ...
    const totalSeconds = resultRef.current.flightTimeSeconds;
  ...
  const startFlight = (nextResult: ShotPhysicsResult, config: ShotPhysicsConfig, nextLaunch: LaunchGesture) => {
    stopAnimation();
  ...
  const applySpin = (sidespinRadPerSecond: number, direction: "left" | "right") => {
    ...
    resultRef.current = continuation.result;
    setResult(continuation.result);
    setProgress(Math.min(1, elapsedPhysicsSecondsRef.current / continuation.result.flightTimeSeconds));
  ...
  return (
    ...
    >
      <div style={{ width: "100%", height: "100dvh", maxWidth: 560, margin: "0 auto", position: "relative", overflow: "hidden", background: "#0d3824" }}>
        <StadiumCanvas samples={result.samples} goalDistanceM={GOAL_DISTANCE_M} progress={progress} />
        ...
        <header style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 10, padding: "max(14px, env(safe-area-inset-top, 14px)) 16px 10px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, pointerEvents: "none" }}>
          <div>
            <div style={{ color: "#ffd166", fontSize: 11, fontWeight: 950, letterSpacing: 1.25 }}>CAMINO AL 10 · FASE 3</div>
            <h1 style={{ margin: "2px 0 0", fontSize: "clamp(22px, 6vw, 30px)", lineHeight: 1, textShadow: "0 2px 10px #000" }}>Controla el tiro</h1>
          </div>
        ...
        {phase === "result" && (
          <section role="status" style={{ position: "absolute", top: "25%", left: "50%", transform: "translateX(-50%)", zIndex: 10, width: "min(82%, 360px)", padding: "15px 18px", textAlign: "center", borderRadius: 19, background: "rgba(3,13,20,.82)", border: `2px solid ${resultCopy.accent}`, boxShadow: `0 0 30px ${resultCopy.accent}44`, backdropFilter: "blur(10px)", pointerEvents: "none" }}>
